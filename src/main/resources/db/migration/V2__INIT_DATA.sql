...
INSERT INTO user_types_roles (user_type_id, role_id)
    SELECT ut.id, r.id FROM user_types ut JOIN roles r ON 1=1 WHERE ut.name = 'ADMIN';

INSERT INTO user_types_roles (user_type_id, role_id)
    SELECT ut.id, r.id FROM user_types ut JOIN roles r ON 1=1 WHERE ut.name = 'RESTAURANT_OWNER';

INSERT INTO user_types_roles (user_type_id, role_id)
    SELECT ut.id, r.id FROM user_types ut JOIN roles r ON 1=1
                       WHERE ut.name = 'CUSTOMER' AND r.name in ('VIEW_USER', 'VIEW_RESTAURANT', 'VIEW_MENU_ITEM', 'VIEW_ORDER');
